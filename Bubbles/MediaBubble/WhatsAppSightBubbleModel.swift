...
class WhatsAppSightBubbleModel: MediaBubbleModel, BubbleInfoProtocol, BubbleImageProtocol {
    var cellType: String {
    ...
    required init?(rcMessages: [RCMessage], currentUserId: String) {
        ...
        guard let rcMessage = rcMessages.first,
              let sightContent = rcMessage.content as? UMBWhatsAppSightMessage else {
            return
        }
        
        thumbImage = sightContent.thumbnailImage
...
//        messagecontentview_height = size.height + image_top_space;
